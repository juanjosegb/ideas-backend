...
class Query(graphene.AbstractType):
    ...
    def resolve_user_ideas(self, info, user_id):
        ...
        if isFollowed:
            ...
            return Idea.objects.filter(Q(creator=user_id), Q(visibility="PUBLIC")).order_by('created_at')

    @login_required
    def resolve_timeline_ideas(self, info):
        user = info.context.user
        followed = Follow.objects.filter(follower=user, pending_request=False, approved=True).values_list("followed")
        return Idea.objects.filter(Q(creator=user) | Q(creator=followed), ~Q(visibility="PRIVATE")).order_by('created_at')
